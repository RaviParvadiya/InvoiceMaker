...
import { updateUser } from "../../api/baseUrl";
...
function ClientEditModal(props) {
  ...
  const onEditHandler = useCallback(() => {
    ...
    updateUser(clientForm._id, clientForm)
      .then((response) => {
        console.log("User updated successfully", response.data);
        dispatch(onConfirmEditClient(response.data));
  ...
  useEffect(() => {
    if (editedID !== null) {
      setAnimate(true);
      const isFindIndex = clients.findIndex((client) => client._id === editedID);
      if (isFindIndex !== -1) {
