...
import { fetchInvoices } from "../../api/baseUrl";
...
function InvoiceTable({ showAdvanceSearch = false }) {
  ...
  const invoices = useMemo(() => {
    ...
    if (searchForm.invoiceName?.trim()) {
      filterData = filterData.filter((invoice) =>
        invoice.invoiceName.includes(searchForm.invoiceName)
      );
    }
  ...
  useEffect(() => {
    fetchInvoices()
      .then((response) => {
        ...
        dispatch(setAllInvoice(response.data));
        dispatch(setAllInvoiceDetailList(response.data));
        ...
        <div className="bg-white rounded-xl px-3 py-3 mb-3">
          ...
          <div className="flex w-full flex-col sm:flex-row">
            <div className="mb-2 sm:mb-0 sm:text-left text-default-color flex flex-row font-title flex-1 px-2">
              ...
              <input
                autoComplete="nope"
                value={searchForm.invoiceName}
                placeholder="Invoice Name"
                className={defaultSearchStyle}
                onChange={(e) => handlerSearchValue(e, "invoiceName")}
              />
            </div>
          </div>
        </div>
        ...
        <div>
          {currentItems &&
            currentItems.map((invoice, index) => (
              <div className={defaultTdWrapperStyle} key={`${index}__${invoice._id}`}>
